...
export class TasksService {
  ...
  async resume(taskId: string): Promise<Task> {
    ...
    try {
      await fetch(
        `${this.configService.get<string>('FOLONITE_DESKTOP_BASE_URL')}/input-tracking/stop`,
        { method: 'POST' },
      );
    } catch (error) {
  ...
  async takeOver(taskId: string): Promise<Task> {
    ...
    try {
      await fetch(
        `${this.configService.get<string>('FOLONITE_DESKTOP_BASE_URL')}/input-tracking/start`,
        { method: 'POST' },
      );
    } catch (error) {
