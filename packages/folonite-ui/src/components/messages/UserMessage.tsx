...
export function UserMessage({ group, messageIdToIndex }: UserMessageProps) {
  if (messageIdToIndex[group.messages[0].id] === 0) {
    return (
      <div className="sticky top-0 z-10 bg-folonite-bronze-light-4">
        <div className="border-folonite-bronze-light-7 flex items-start justify-start gap-2 border px-4 py-3 bg-folonite-bronze-light-2 rounded-t-lg">
          ...
          <div>
            {group.messages.map((message) => (
              ...
              >
                ...
                <div className="bg-folonite-bronze-light-4 space-y-2 rounded-md px-2 py-1">
                  {message.content.map((block, index) => (
                    <div
                      key={index}
                      className="text-folonite-bronze-light-12 text-sm"
                    >
  ...
  return (
    <div className="bg-folonite-bronze-light-3 flex items-start justify-end gap-2 px-4 py-3 border-x border-folonite-bronze-light-7">
      <div>
        {group.messages.map((message) => (
          <div
            key={message.id}
