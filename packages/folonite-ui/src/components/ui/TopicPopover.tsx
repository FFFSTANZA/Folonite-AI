...
export const TopicPopover: React.FC<TopicPopoverProps> = ({
  ...
  const modifiedChildren = React.Children.map(children, (child) => {
    ...
    // Replace text-folonite-bronze-light-11 with text-folonite-bronze-light-12 when open
    const updatedClassName = isOpen 
      ? existingClassName.replace('text-folonite-bronze-light-11', 'text-folonite-bronze-light-12')
      : existingClassName;
  ...
  return (
    <div className="relative" ref={popoverRef}>
      <div onClick={handleToggle} className={isOpen ? "bg-folonite-bronze-light-1 rounded-full" : ""}>
        {modifiedChildren}
      </div>
    </div>
  );
