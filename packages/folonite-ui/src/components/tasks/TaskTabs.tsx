...
const TAB_CONFIGS: Record<TabKey, TabConfig> = {
  ALL: {
    label: "All",
    icon: ListViewIcon,
    color: "text-folonite-bronze-light-10",
...
export const TaskTabs: React.FC<TaskTabsProps> = ({
  ...
  return (
    <div className="border-folonite-bronze-light-7 mb-6 border-b">
          ...
          return (
            <button
              ...
              className={`flex cursor-pointer items-center space-x-2 border-b-2 px-4 py-3 whitespace-nowrap transition-colors ${
                isActive
                  ? "border-folonite-bronze-dark-7 text-folonite-bronze-dark-7"
                  : "text-folonite-bronze-light-10 hover:text-folonite-bronze-dark-7 border-transparent"
              }`}
            >
              <HugeiconsIcon
                icon={config.icon}
                className={`h-4 w-4 ${isActive ? "text-folonite-bronze-dark-7" : config.color}`}
              />
              ...
              {count > 0 && (
                <span
                  className={`rounded-full px-2 py-0.5 text-xs ${
                    isActive
                      ? "bg-folonite-bronze-dark-7 text-white"
                      : "bg-folonite-bronze-light-7 text-folonite-bronze-light-11"
                  }`}
                >
                  {count}
                </span>
