...
@Injectable()
export class ProxyService implements FoloniteAgentService {
  private readonly openai: OpenAI;
  private readonly logger = new Logger(ProxyService.name);

  constructor(private readonly configService: ConfigService) {
    const proxyUrl = this.configService.get<string>('FOLONITE_LLM_PROXY_URL');

    if (!proxyUrl) {
      this.logger.warn(
        'FOLONITE_LLM_PROXY_URL is not set. ProxyService will not work properly.',
      );
    }
  ...
  /**
   * Convert Folonite messages to Chat Completion format
   */
