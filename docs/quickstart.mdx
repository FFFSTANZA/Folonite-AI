...
Folonite can be deployed in several ways depending on your needs:
...
<Steps>
  <Step title="Visit the Template">
    Click the Deploy Now button in the Folonite template on Railway.
  </Step>
  <Step title="Add Anthropic Key">
    Enter either your `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, or `GEMINI_API_KEY` for the folonite-agent resource.
  </Step>
...
- **Folonite Desktop**: Containerized Linux environment
...
Navigate to [http://localhost:9992](http://localhost:9992) to access the Folonite UI.

**Two ways to interact:**
1. **Tasks**: Enter task descriptions to have Folonite work autonomously
...
<Note>
  **🔐 Password Manager Support**: Folonite can handle authentication automatically when you install a password manager extension. See our [password management guide](/guides/password-management) for setup instructions.
</Note>
...
Now let's see Folonite in action! Try these example tasks:
...
# Just the agent
docker-compose -f docker/docker-compose.yml logs -f folonite-agent
```
...
curl -X POST http://localhost:9990/computer-use \
  -H "Content-Type: application/json" \
  -d '{"action": "type_text", "text": "Hello, Folonite!"}'
```
...
<AccordionGroup>
  ...
  <Accordion title="Agent errors or no response">
    ...
    ```bash 
    cat docker/.env
    docker-compose -f docker/docker-compose.yml logs folonite-agent 
    ```
