---
title: "Takeover Mode"
description: "Take control of the desktop when you need to guide or assist Folonite"
---

# Takeover Mode: Human-AI Collaboration

Takeover mode lets you take control of the desktop to help Folonite when needed. There are two ways to use it:
...
All your actions during takeover (clicks, drags, scrolls, typing, key presses) are automatically logged in the same unified action space that the agent uses. This means Folonite understands and learns from everything you do.
...
<Steps>
  <Step title="Open Task Detail View">
    While Folonite is working on a task, click on the task to open the detail view.
  </Step>
  ...
  <Step title="Guide Folonite">
    Perform the necessary actions to get past the obstacle or show the correct path.
  </Step>
  
  <Step title="Release Control">
    Click to release control and let Folonite continue from where you left off.
  </Step>
</Steps>

### Method 2: Automatic When Help Needed

When Folonite sets a task status to "needs help":
- Takeover mode is automatically enabled
- You'll see a notification that Folonite needs assistance
- Take control to help resolve the issue
- Folonite will continue once you release control
...
<Card title="Handling Unexpected Situations" icon="exclamation-triangle">
  **Scenario**: Folonite encounters an error or gets stuck
  
  **Steps**:
  1. Notice Folonite is struggling
  2. Take control to resolve the issue
  3. Guide it past the problem
  4. Explain what went wrong in chat
  5. Return control to let Folonite continue
...
<Card title="Show Don't Tell" icon="graduation-cap">
  ...
  **Steps**:
  ...
  4. Return control
  5. Ask Folonite to repeat the process
...
- **Test Understanding**: Ask Folonite to summarize what it learned

### Don'ts ❌

- **Enter Data You Don't Want Captured**: Screenshots are taken of all actions
- **Skip Chat Explanations**: Context helps Folonite learn patterns
...
<Note>
  **No Need to Move Slowly**: Folonite captures the state before and after each action, so you can work at normal speed.
</Note>


## Summary

Takeover mode provides flexibility when you need to guide Folonite or handle situations it can't manage alone. Whether you're navigating complex interfaces, recovering from errors, or teaching new workflows, takeover mode ensures you're always in control when needed.