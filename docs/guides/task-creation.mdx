...
# Creating and Managing Tasks in Folonite

This guide will walk you through everything you need to know about creating tasks that Folonite can execute effectively, and managing them through their lifecycle.

## Understanding Tasks

A task is any job you want Folonite to complete. Tasks can be:
...
Folonite has powerful file handling capabilities that make it perfect for document processing and data analysis tasks.
...
- **Document Processing**: Upload PDFs, spreadsheets, or documents for Folonite to analyze
...
<Note>
  **Game Changer**: Folonite can read entire files, including PDFs, directly into the LLM context. This means it can process large amounts of data quickly and understand complex documents without manual extraction.
</Note>
...
<Steps>
  ...
  <Step title="Start Task">
    Press the arrow button or hit Enter. Folonite will start loading and begin working on your task.
  </Step>
</Steps>
...
(Folonite handles all authentication including 2FA automatically via password manager)
```
...
Watch Folonite work through the task detail viewer:

- **Green dot**: Task is actively running
- **Status messages**: Current step being executed
- **Desktop view**: See what Folonite sees in real-time

#### Chat Messages

Folonite provides updates like:
```
...
(Note: Folonite handles all authentication including 2FA automatically using the built-in password manager)
...
After each task:
1. Review the approach Folonite took
...
<Note>
  **Pro Tip**: Start with simple tasks to understand Folonite's capabilities, then gradually increase complexity as you learn what works best.
</Note>