...
description: "How Folonite handles authentication automatically using password managers"
---

# Automated Authentication with Folonite

Folonite can handle authentication automatically - including passwords, 2FA, and even complex multi-step authentication flows - when you set up a password manager extension.
...
Folonite comes with 1Password built-in and supports any browser-based password manager extension. It can:
...
<Steps>
  <Step title="Install 1Password Extension">
    1. Go to the Desktop tab in Folonite UI
    2. Open Firefox
    3. Install the 1Password extension from the Firefox Add-ons store
    4. Sign in to your 1Password account (or create a dedicated one for Folonite)
  </Step>
  
  <Step title="Configure Vaults">
    In your 1Password admin panel:
    1. Create a vault called "Folonite Automation"
    2. Add the credentials Folonite needs
    3. Share the vault with Folonite's account
...
<Steps>
  <Step title="Create Service Accounts">
    Set up dedicated service accounts for Folonite:
    ```
    - folonite-finance@company.com (banking portals)
    - folonite-hr@company.com (HR systems)
    - folonite-ops@company.com (operational tools)
    ```
  </Step>
...
<CardGroup cols={2}>
  <Card title="Least Privilege" icon="shield-halved">
    Only share credentials Folonite needs for specific tasks
  </Card>
  ...
  <Card title="Regular Reviews" icon="calendar-check">
    Audit Folonite's credential access monthly
  </Card>
</CardGroup>
...
- Folonite's IP whitelisted at banks
...
2. Mount certificate file to Folonite
...
<AccordionGroup>
  ...
  <Accordion title="2FA code rejected">
    ...
    **Fix:**
    ```bash
    # Sync system time
    docker exec folonite-desktop ntpdate -s time.nist.gov
    ```
  </Accordion>
...
# Folonite handles all authentication automatically
```
...
✅ **DO:**
- Use dedicated service accounts for Folonite
- Organize credentials in logical vaults
- Enable 2FA on all accounts (Folonite handles it!)
- Rotate passwords regularly
- Monitor access logs

❌ **DON'T:**
- Share personal credentials with Folonite
...
<Note>
  **Game Changer**: With proper password manager setup, Folonite can handle even the most complex authentication flows automatically. No more manual intervention for 2FA, no more sharing passwords insecurely, and no more authentication bottlenecks in your automation workflows!
</Note>