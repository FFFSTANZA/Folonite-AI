---
title: 'Task UI'
description: 'Documentation for the Folonite Task UI'
---

## Folonite Task UI

The Folonite Task UI provides a web-based interface for interacting with the Folonite agent system. It combines a action feed with an embedded noVNC viewer, allowing you to watch it perform task on the desktop in real-time.

<img src="/static/chat-ui-overview.png" alt="Folonite Task Detail" className="w-full max-w-4xl" />

## Accessing the UI

When running the full Folonite agent system, the Task UI is available at:
...
The task interface displays different types of messages based on Folonite's content block structure:
...
The Folonite Task UI is built with:
...
1. Ensure all Folonite services are running
...
1. Ensure the Folonite container is running
