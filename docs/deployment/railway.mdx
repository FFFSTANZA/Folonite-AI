---
title: "Deploying Folonite on Railway"
description: "Comprehensive guide to deploying the full Folonite stack on Railway using the official 1-click template"
---

> **TL;DR –** Click the button below, add your AI API key (Anthropic, OpenAI, or Google), and your personal Folonite instance will be live in ~2 minutes.

[![Deploy on Railway](https://railway.com/button.svg)](https://railway.com/deploy/folonite?referralCode=L9lKXQ)

---

## Why Railway?

Railway provides a zero-ops PaaS experience with private networking and per-service logs that perfectly fits Folonite’s multi-container architecture. The official template wires every service together using the latest container images pushed to the `edge` branch.
...
| ---------------- | -------------------------------------------------------------------- | ---- | -------- | ------------------------------------ |
| **folonite-ui**   | `ghcr.io/folonite-ai/folonite-ui:edge`                                 | 9992 | **Yes**  | Next.js web UI rendered to the world |
| **folonite-agent**| `ghcr.io/folonite-ai/folonite-agent:edge`                              | 9991 | No       | Task orchestration & LLM calls       |
| **folonite-desktop**| `ghcr.io/folonite-ai/folonite-desktop:edge`                          | 9990 | No       | Containerised Ubuntu + XFCE desktop  |
| **postgres**     | `postgres:14-alpine`                                                 | 5432 | No       | Persistence layer                    |

All internal traffic flows through Railway’s [private networking](https://docs.railway.com/guides/private-networking). Only `folonite-ui` is assigned a public domain.
...
<Steps>
  <Step title="1. Open the Template">
    Click the **Deploy on Railway** button above or visit [https://railway.com/deploy/folonite?referralCode=L9lKXQ](https://railway.com/deploy/folonite?referralCode=L9lKXQ).
  </Step>
  <Step title="2. Configure Environment">
    For the folonite-agent resource, add your AI API key (choose at least one):
  ...
  <Step title="4. Launch Folonite">
    When the build logs show *"folonite-ui: ready"*, click the generated URL (e.g. `https://folonite-ui-prod.up.railway.app`). You should see the task interface. Create a task and watch the desktop stream!  
...
| ------- | ------------ | ---- |
| Web UI shows “connecting…” | Desktop not ready or private networking mis-config | Wait for `folonite-desktop` container to finish starting, or restart service |
