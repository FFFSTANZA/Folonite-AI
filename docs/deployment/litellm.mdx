...
description: "Use any LLM provider with Folonite through LiteLLM proxy"
---

# Connect Any LLM to Folonite with LiteLLM

LiteLLM acts as a unified proxy that lets you use 100+ LLM providers with Folonite - including Azure OpenAI, AWS Bedrock, Anthropic, Hugging Face, Ollama, and more. This guide shows you how to set up LiteLLM with Folonite.
...
## Quick Start with Folonite's Built-in LiteLLM Proxy

Folonite includes a pre-configured LiteLLM proxy service that makes it easy to use any LLM provider. Here's how to set it up:
...
<Steps>
  <Step title="Use Docker Compose with Proxy">
    ...
    # Start Folonite with LiteLLM proxy
    docker-compose -f docker/docker-compose.proxy.yml up -d
    ```
    
    This automatically:
    - Starts the `folonite-llm-proxy` service on port 4000
    - Configures the agent to use the proxy via `FOLONITE_LLM_PROXY_URL`
  ...
  <Step title="Customize Model Configuration">
    ...
    ```yaml
    # packages/folonite-llm-proxy/litellm-config.yaml
    model_list:
      # Add Azure OpenAI
  ...
  <Step title="Verify Models are Available">
    The Folonite agent automatically queries the proxy for available models:
    
    ```bash
    # Check available models through Folonite API
...
```mermaid
graph LR
    A[Folonite UI] -->|Select Model| B[Folonite Agent]
    B -->|FOLONITE_LLM_PROXY_URL| C[LiteLLM Proxy :4000]
...
1. **folonite-llm-proxy Service**: A LiteLLM instance running in Docker that:
   - Runs on port 4000 within the Folonite network
   - Uses the config from `packages/folonite-llm-proxy/litellm-config.yaml`
   - Inherits API keys from environment variables

2. **Agent Integration**: The Folonite agent:
   - Checks for `FOLONITE_LLM_PROXY_URL` environment variable
...
# Use "smart-model" in Folonite config
```
...
services:
  folonite-agent:
    environment:
      ...
      - FOLONITE_LLM_PROXY_URL=http://your-litellm-server:4000
    # ... rest of config
```
...
export FOLONITE_LLM_PROXY_URL=http://your-litellm-server:4000
...
# Then start Folonite with:
export FOLONITE_LLM_PROXY_URL=http://localhost:4000
docker-compose up -d
```
...
<AccordionGroup>
  ...
  <Accordion title="Authentication errors">
    Verify master key in both LiteLLM and Folonite:
    ```bash
    # Test LiteLLM
...
```yaml
# Optimize for Folonite workloads
router_settings:
  routing_strategy: "latency-based"
...
<Note>
  **Pro tip:** Start with a single provider, then add more as needed. LiteLLM makes it easy to switch or combine models without changing Folonite configuration.
</Note>